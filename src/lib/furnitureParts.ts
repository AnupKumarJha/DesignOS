...
function generateCaseParts(item: Furniture): FurniturePart[] {
  ...
  const add = (type: FurniturePartType, name: string, position: FurniturePart['position'], size: FurniturePart['size'], options: Partial<FurniturePart> = {}) => {
    const typeIndex = parts.filter((part) => part.type === type).length + 1;
    parts.push({
      id: `${item.id}:${type}:${typeIndex}`,
      type,
      name,
      position,
      size,
      thickness: options.thickness ?? panelThickness(type),
      materialId: options.materialId ?? internalMaterialId,
  ...
  add('left_panel', 'Left Side Panel', { x: -w / 2 + PANEL / 2, y: 0, z: 0 }, { width: PANEL, height: h, depth: d }, { materialId: exteriorMaterialId });
  add('right_panel', 'Right Side Panel', { x: w / 2 - PANEL / 2, y: 0, z: 0 }, { width: PANEL, height: h, depth: d }, { materialId: exteriorMaterialId });
  add('top_panel', 'Top Panel', { x: 0, y: h / 2 - PANEL / 2, z: 0 }, { width: w, height: PANEL, depth: d }, { materialId: exteriorMaterialId });
  add('bottom_panel', 'Bottom Panel', { x: 0, y: -h / 2 + PANEL / 2, z: 0 }, { width: w, height: PANEL, depth: d }, { materialId: exteriorMaterialId });
  add('back_panel', 'Inset Back Panel', { x: 0, y: 0, z: -d / 2 + BACK / 2 }, { width: innerW, height: innerH, depth: BACK }, { materialId: internalMaterialId, thickness: BACK });
  ...
  for (let index = 0; index < partitionCount; index += 1) {
    ...
    add('vertical_partition', `Vertical Partition ${index + 1}`, { x, y: 0, z: -PANEL / 2 }, { width: PANEL, height: innerH, depth: innerD }, { materialId: internalMaterialId });
  }

  const shelfCount = Math.max(0, item.shelfCount ?? defaultShelfCount(item));
  for (let index = 0; index < shelfCount; index += 1) {
    const y = -innerH / 2 + (innerH / (shelfCount + 1)) * (index + 1);
    add('shelf', `Adjustable Shelf ${index + 1}`, { x: 0, y, z: -PANEL / 2 }, { width: innerW, height: PANEL, depth: innerD }, { materialId: internalMaterialId });
  }

  if ((item.hasHangingRod ?? ['WARDROBE', 'CABINET_TALL'].includes(item.type)) && h > 1300) {
    add('hanging_rod', 'Hanging Rod', { x: 0, y: h * 0.22, z: d * 0.04 }, { width: Math.max(120, innerW - 120), height: 24, depth: 24 }, { metalness: 0.75, roughness: 0.16 });
  }

  if (item.hasBasket || item.catalogItemId === 'pullout_unit') {
    add('basket', 'Wire Basket Pullout', { x: 0, y: -h * 0.12, z: d * 0.08 }, { width: innerW - 80, height: Math.min(220, h * 0.32), depth: innerD - 70 }, { mechanism: 'pullout', metalness: 0.55, roughness: 0.2 });
  }
  ...
  if (drawerCount > 0) {
    ...
    for (let index = 0; index < drawerCount; index += 1) {
      ...
      const frontId = `${item.id}:drawer_front:${index + 1}`;
      add('drawer_front', `Drawer Front ${index + 1}`, { x: 0, y, z: d / 2 + SHUTTER / 2 }, { width: w - GAP * 2, height: frontH, depth: SHUTTER }, { materialId: exteriorMaterialId, mechanism: 'slide' });
      add('drawer_box', `Drawer Box ${index + 1}`, { x: 0, y, z: d * 0.08 }, { width: innerW - 50, height: Math.max(70, frontH - 48), depth: innerD * 0.74 }, { parentPartId: frontId, mechanism: 'slide', materialId: internalMaterialId });
      add('runner', `Runner Pair ${index + 1}`, { x: 0, y: y - frontH * 0.25, z: d * 0.1 }, { width: innerW - 34, height: 18, depth: innerD * 0.82 }, { parentPartId: frontId, mechanism: 'slide', metalness: 0.7, roughness: 0.2 });
      if (item.hasHandle !== false) {
        add('handle', `Drawer Handle ${index + 1}`, { x: 0, y, z: d / 2 + 44 }, { width: Math.min(260, w * 0.42), height: 18, depth: 18 }, { parentPartId: frontId, mechanism: 'slide', metalness: 0.72, roughness: 0.16 });
      }
    }
    ...
    for (let index = 0; index < shutterCount; index += 1) {
      ...
      add('shutter', `Shutter ${index + 1}`, { x, y: 0, z: d / 2 + SHUTTER / 2 }, { width: shutterW, height: h - GAP * 2, depth: SHUTTER }, { materialId: exteriorMaterialId, mechanism: 'swing', hingeSide });
      ...
      for (let hingeIndex = 0; hingeIndex < hingeCount; hingeIndex += 1) {
        ...
        add('hinge', `Hinge ${index + 1}.${hingeIndex + 1}`, { x: x + (hingeSide === 'left' ? -shutterW / 2 + 18 : shutterW / 2 - 18), y: hingeY, z: d / 2 + 38 }, { width: 34, height: 76, depth: 18 }, { parentPartId: shutterId, mechanism: 'swing', hingeSide, metalness: 0.75, roughness: 0.18 });
      }
      if (item.hasHandle !== false) {
        add('handle', `Shutter Handle ${index + 1}`, { x: x + (hingeSide === 'left' ? shutterW * 0.32 : -shutterW * 0.32), y: 0, z: d / 2 + 44 }, { width: 18, height: Math.min(220, h * 0.34), depth: 18 }, { parentPartId: shutterId, mechanism: 'swing', hingeSide, metalness: 0.72, roughness: 0.16 });
      }
    }
  }

  if ((item.skirtingHeight ?? 0) > 0) {
    add('skirting', 'Recessed Skirting', { x: 0, y: -h / 2 - (item.skirtingHeight ?? 0) / 2, z: 0 }, { width: Math.max(80, w - 80), height: item.skirtingHeight ?? 80, depth: Math.max(80, d - 80) }, { materialId: undefined });
  }
  if (['CABINET_BASE', 'SINK_UNIT', 'VANITY'].includes(item.type)) {
    add('countertop', item.type === 'VANITY' ? 'Vanity Countertop' : 'Countertop', { x: 0, y: h / 2 + 28, z: 0 }, { width: w + 60, height: 56, depth: d + 70 }, { materialId: 'stone_granite_black' });
  }
  if (item.type === 'SINK_UNIT' || item.catalogItemId === 'sink_unit') {
    add('sink', 'Sink Bowl', { x: 0, y: h / 2 + 66, z: d * 0.08 }, { width: Math.min(520, w * 0.62), height: 90, depth: d * 0.48 }, { metalness: 0.22, roughness: 0.2 });
  }
